...
export class Heap {
    ...
    // Not used with garbage collection
    private free(key: Key) {
    ...
    write(key: Key, val: Value) {
        ...
        if (data && data.length > key.offset && key.offset >= 0) {
            ...
            throw error(`Uninitialized heap location ${key.base} and/or illegal offset ${key.offset} for write`);
        }
    }
    ...
    read(key: Key): Value {
        ...
        if (data && data.length > key.offset && key.offset >= 0) {
            ...
            throw error(`Uninitialized heap location ${key.base} and/or illegal offset ${key.offset} for read`);
        }
    }
}
...
function evalInstr(instr: bril.Instruction, state: State): Action {
  ...
  case "free": {
    throw error(`Free: not using garbage collector`);
  }
